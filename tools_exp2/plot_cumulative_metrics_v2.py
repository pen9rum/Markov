...
Same visual style as exp3/plot_cumulative_metrics.py.

Input:  exp2(generation_blind)/analysis_results/cumulative_metrics.csv
Output: exp2(generation_blind)/plots/
          cumulative_{model}.png  (per-model 2x2)
          cumulative_overall.png  (all models)
          cumulative_zoom_overlap_strict_overall.png

Usage:
  python tools_exp2/plot_cumulative_metrics_v2.py
"""
import os
import csv
...
# exp2 column name for overlap (exp3 uses 'overlap_cum')
    ...
    for model in models:
        ...
        for c in combos:
            ...
            for ax_i, metric, ylabel, ylim, bl in [
                ...
                (2, OVERLAP_COL,  'Cumulative Overlap', (0, 1.05), 1/3),
        ...
        titles = [
            f'{model} — Cumulative CE (XYZ)',
            f'{model} — Cumulative MSE (XYZ)',
            f'{model} — Cumulative Overlap (XYZ)',
            f'{model} — Cumulative Strict (XYZ)',
        ]
...
def plot_overall(rows, outdir):
    ...
    for model in models:
        ...
        for ax_i, metric in enumerate(['ce_cum', 'mse_cum', OVERLAP_COL, 'strict_cum']):
            xs, ys = avg_by_window(m_rows, metric)
            if xs:
                axes[ax_i].plot(xs, ys, marker='o', linewidth=2, color=color, label=model)

    titles = [
        'All Models — Cumulative CE (XYZ)',
        'All Models — Cumulative MSE (XYZ)',
        'All Models — Cumulative Overlap (XYZ)',
        'All Models — Cumulative Strict (XYZ)',
    ]
    for ax, title, ylabel, ylim, bl in zip(axes, titles,
        ['CE', 'MSE', 'Overlap Rate', 'Strict Rate'],
        [None, None, (0, 1.05), (0, 1.05)],
...
def plot_zoomed_overlap_strict(rows, outdir):
    ...
    for model in models:
        m_rows = [r for r in rows if r['model'] == model]
        color  = MODEL_COLORS.get(model)
        xs_o, ys_o = avg_by_window(m_rows, OVERLAP_COL)
    ...
    setup_ax(axes[0], 'All Models — Cumulative Overlap (Zoom) (XYZ)',
             'Overlap Rate', overlap_ylim, 1/3, windows)
    setup_ax(axes[1], 'All Models — Cumulative Strict (Zoom) (XYZ)',
