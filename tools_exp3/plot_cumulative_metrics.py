"""
Exp3 — Plot cumulative metrics per model and overall.
Mirrors exp2/plot_cumulative_metrics.py.
...
Usage:
  python tools_exp3/plot_cumulative_metrics.py --markov-set qrs
"""
import os
import csv
import argparse
...
def plot_per_model(rows, outdir, markov_set):
...
def plot_overall(rows, outdir, markov_set):
    models  = sorted({r['model'] for r in rows})
    windows = sorted({int(r['window_idx']) for r in rows})
...
def plot_zoomed_overlap_strict(rows, outdir, markov_set):
    """Extra detailed view for small fluctuations. Original plots are kept unchanged."""
    models  = sorted({r['model'] for r in rows})
    windows = sorted({int(r['window_idx']) for r in rows})
    ...
    # Overlap: user-requested zoom around high-accuracy region
    overlap_ylim = (0.9, 1.0)
    # Strict: auto tight zoom so constant or near-constant lines are still visible
...
def main():
    ...
    plot_per_model(rows, outdir, ms)
    plot_overall(rows, outdir, ms)
    plot_zoomed_overlap_strict(rows, outdir, ms)
    print(f'\nDone. Plots → {outdir}')
